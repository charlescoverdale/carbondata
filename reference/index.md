# Package index

## Compliance Markets

ETS allowances, prices, and allocation data

- [`co2_euets_price()`](https://charlescoverdale.github.io/carbondata/reference/co2_euets_price.md)
  : EU ETS allowance (EUA) auction settlement prices
- [`co2_euets_allocations()`](https://charlescoverdale.github.io/carbondata/reference/co2_euets_allocations.md)
  : EU ETS free allowance allocations
- [`co2_euets_emissions()`](https://charlescoverdale.github.io/carbondata/reference/co2_euets_emissions.md)
  : EU ETS verified emissions
- [`co2_euets_surrendered()`](https://charlescoverdale.github.io/carbondata/reference/co2_euets_surrendered.md)
  : EU ETS compliance status and cumulative surrendered units
- [`co2_euets_installations()`](https://charlescoverdale.github.io/carbondata/reference/co2_euets_installations.md)
  : EU ETS installation registry
- [`co2_euets_files()`](https://charlescoverdale.github.io/carbondata/reference/co2_euets_files.md)
  : List the EU ETS data file versions bundled with this package
- [`co2_ukets()`](https://charlescoverdale.github.io/carbondata/reference/co2_ukets.md)
  : UK ETS verified emissions and surrenders
- [`co2_ukets_allocations()`](https://charlescoverdale.github.io/carbondata/reference/co2_ukets_allocations.md)
  : UK ETS free allocations
- [`co2_california_prices()`](https://charlescoverdale.github.io/carbondata/reference/co2_california_prices.md)
  : California Cap-and-Trade auction settlement prices
- [`co2_california_caps()`](https://charlescoverdale.github.io/carbondata/reference/co2_california_caps.md)
  : California overall emissions caps
- [`co2_rggi_allowances()`](https://charlescoverdale.github.io/carbondata/reference/co2_rggi_allowances.md)
  : RGGI allowance distribution by state and year
- [`co2_rggi_state_proceeds()`](https://charlescoverdale.github.io/carbondata/reference/co2_rggi_state_proceeds.md)
  : RGGI cumulative state proceeds by auction
- [`co2_icap_prices()`](https://charlescoverdale.github.io/carbondata/reference/co2_icap_prices.md)
  : ICAP Allowance Price Explorer

## Voluntary Markets

Verra, Gold Standard, ACR, CAR, and offset databases

- [`co2_vrod()`](https://charlescoverdale.github.io/carbondata/reference/co2_vrod.md)
  : Berkeley Voluntary Registry Offsets Database
- [`co2_offsets_db()`](https://charlescoverdale.github.io/carbondata/reference/co2_offsets_db.md)
  : CarbonPlan OffsetsDB (via S3 parquet)
- [`co2_cad_trust()`](https://charlescoverdale.github.io/carbondata/reference/co2_cad_trust.md)
  : Climate Action Data Trust (not supported in v0.1.0)

## Policy and Pricing

World Bank and RFF carbon pricing data

- [`co2_world_bank()`](https://charlescoverdale.github.io/carbondata/reference/co2_world_bank.md)
  : World Bank Carbon Pricing Dashboard
- [`co2_rff_pricing()`](https://charlescoverdale.github.io/carbondata/reference/co2_rff_pricing.md)
  : RFF World Carbon Pricing Database

## General

Market index and cache

- [`co2_markets()`](https://charlescoverdale.github.io/carbondata/reference/co2_markets.md)
  : List supported carbon markets
- [`co2_cache_info()`](https://charlescoverdale.github.io/carbondata/reference/co2_cache_info.md)
  : Inspect the local carbondata cache
- [`co2_clear_cache()`](https://charlescoverdale.github.io/carbondata/reference/co2_clear_cache.md)
  : Clear the carbondata cache

## Package

- [`carbondata`](https://charlescoverdale.github.io/carbondata/reference/carbondata-package.md)
  [`carbondata-package`](https://charlescoverdale.github.io/carbondata/reference/carbondata-package.md)
  : carbondata: Access Carbon Market Data from Emissions Trading Systems
  and Voluntary Registries
